...
- `continuity_query`, `narrative_query`, `actor_query`, the host-selected stable native catalog, exposure guidance, and prior authoritative receipts.
...
5. Call `mechanic_resolve` only if the host-selected catalog includes it and current phase/authority guidance permits the call; exposure alone never authorizes it.
...
10. If a reasonable player choice leaves the current Atlas, request
    `campaign_expansion` action `context`. Give its signed capsule to a bounded
    worker with no tools or persistence; validate and attest the returned
    `emergent_episode` proposal; then use the unchanged attestation and a
    checksummed parent dependency through the native Pack lifecycle to create,
    finalize, import, and activate the new child version. Do not narrate the
    proposal as canon before activation.
11. Update fronts, threads, clues, and character-arc progress only through legal
    status transitions backed by typed evidence that exists on the current
    branch. A player arc can complete only by citing a declared opportunity;
    preserve open choice and never author an already-completed outcome.
